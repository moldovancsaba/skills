...
import type { DataPageInitialData } from "@/lib/server-company-page-data";
...
export default function CompanyDataClient({
  ...
  companyId: string;
  initialData?: DataPageInitialData | null;
}) {
  const PAGE_SIZE = 12;
  ...
  const [items, setItems] = useState<DataItem[]>(() => initialData?.items ?? []);
  const [loading, setLoading] = useState(!initialData);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [selectedDataId, setSelectedDataId] = useState<string | null>(null);
  const [isOwner, setIsOwner] = useState(Boolean(initialData?.isOwner));
  const [fileCount, setFileCount] = useState(() => initialData?.fileCount ?? 0);
  ...
  useEffect(() => {
    ...
    setSources(initialData.sourceItems);
  ...
  }, [companyId, initialData, router, setCompany, loadAllData]);
