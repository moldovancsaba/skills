...
type ListCompanyFlashcardsPageOptions = {
  limit?: number;
  offset?: number;
  searchQuery?: string;
  kind?: string | null;
  intelligenceType?: "INTERNAL" | "COMPETITOR" | null;
  hashtags?: string[];
};

function buildCompanyFlashcardWhere(
  companyId: string,
  {
    searchQuery,
    kind,
    intelligenceType,
    hashtags,
  }: Omit<ListCompanyFlashcardsPageOptions, "limit" | "offset"> = {},
  ...
  if (kind && kind !== "ALL") {
    where.kind = kind as FlashcardKind;
  }

  if (intelligenceType) {
    where.intelligenceType = intelligenceType;
  }

  const normalizedTags = Array.isArray(hashtags)
    ? [...new Set(hashtags.map((tag) => normalizeText(tag)).filter(Boolean))]
    : [];
...
function serializeCompanyFlashcardsForKnowmore(
  flashcards,
  researchHarvestIds: Set<string>,
) {
  return flashcards.map((flashcard) => {
    ...
    return {
      ...
      ischecklistResearch: flashcard.sources.some(
        (source) => source.sourceType === "SOURCE" && researchHarvestIds.has(source.sourceId),
      ),
    };
  });
}

export async function listCompanyFlashcardsPage(
  companyId: string,
  options: ListCompanyFlashcardsPageOptions = {},
) {
  const limit = Math.max(1, Math.min(50, Number(options.limit || 12)));
  const offset = Math.max(0, Number(options.offset || 0));
  const where = buildCompanyFlashcardWhere(companyId, options);
  ...
  const items = serializeCompanyFlashcardsForKnowmore(flashcards, researchHarvestIds);
