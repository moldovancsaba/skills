import { NextRequest, NextResponse } from "next/server";
import { verifyMembership } from "@/lib/permissions";
import { getVisitorTaxonomy } from "@/lib/visitor-blueprints";
import { evaluateVisitorQualityGate } from "@/lib/visitor-quality-gate";

export const dynamic = "force-dynamic";

function asRecord(value: unknown): Record<string, unknown> {
  return value && typeof value === "object" && !Array.isArray(value) ? (value as Record<string, unknown>) : {};
}

function asString(value: unknown) {
  return typeof value === "string" ? value.trim() : "";
}

export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ visitorKey: string }> },
) {
  const body = asRecord(await request.json().catch(() => null));
  const companyId = asString(body.companyId);
  if (!companyId) return NextResponse.json({ ok: false, error: "companyId is required" }, { status: 400 });
  const auth = await verifyMembership(request, companyId, "MEMBER");
  if (auth.error) return auth.error;
  const { visitorKey } = await params;
  const destinationKey = asString(body.destinationKey) || undefined;
  const contentType = asString(body.contentType);
  if (!contentType) return NextResponse.json({ ok: false, error: "contentType is required" }, { status: 400 });

  try {
    const taxonomy = await getVisitorTaxonomy(companyId, visitorKey, destinationKey);
    const gateResult = evaluateVisitorQualityGate({
      taxonomy,
      contentType,
      sourceUrl: asString(body.sourceUrl),
      extractedFacts: body.extractedFacts,
      metadata: body.metadata,
    });
    return NextResponse.json({ ok: true, visitorKey, gateResult });
  } catch (error) {
    return NextResponse.json({ ok: false, error: String(error) }, { status: 400 });
  }
}
