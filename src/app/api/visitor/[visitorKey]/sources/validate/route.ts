...
import { validateVisitorSourceGraph } from "@/lib/visitor-source-graph";
...
export async function GET(
  ...
  try {
    ...
    const validation = await validateVisitorSourceGraph(companyId, visitorKey, destinationKey);
    return NextResponse.json({ ok: true, visitorKey, validation });
  } catch (error) {
