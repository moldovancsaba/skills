...
import { applyVisitorFeedback, listVisitorRefinementRuns } from "@/lib/visitor-learning";
...
export async function GET(
  ...
  const runs = await listVisitorRefinementRuns(companyId, visitorKey);
  return NextResponse.json({ ok: true, visitorKey, runs });
}
...
export async function POST(
  ...
  return NextResponse.json({ ok: true, visitorKey, run: result.run, rule: result.rule });
}
