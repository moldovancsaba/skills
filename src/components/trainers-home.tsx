...
import type { TrainersLandingSummary } from "@/lib/trainers-landing";
...
export function TrainersHome({
  ...
  companyId: string;
  initialSummary?: TrainersLandingSummary | null;
}) {
  const [loading, setLoading] = useState(!initialSummary);
  const [summary, setSummary] = useState<TrainersLandingSummary | null>(initialSummary);
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const response = await fetch(`/api/trainers/landing?companyId=${encodeURIComponent(companyId)}`);
      const payload = response.ok ? await response.json() : null;
      if (!response.ok || !payload?.destinationKey) {
        throw new Error(String(payload?.error || "Trainers landing summary unavailable."));
      }
      setSummary(payload as TrainersLandingSummary);
    } catch (error) {
      setSummary(null);
      setLoadError(error instanceof Error ? error.message : "Trainers landing summary unavailable.");
    } finally {
      setLoading(false);
    }
  }, [companyId]);
      ...
      <PageShell width="full">
        <Center h={400}>
          <Stack align="center" gap="sm">
            <Loader color="review" />
            <Text c="dimmed">Loading Trainers operator home...</Text>
          </Stack>
        </Center>
      </PageShell>
    );
    ...
    return (
      <PageShell width="full">
        <Stack gap="xl">
          <PageHeader
            title="Trainers"
            description="Dedicated operator home for Trainers workflows, review queues, project delivery, and mission control."
            actions={(
          ...
          />
          <Notice title="Trainers home is temporarily unavailable" icon={Activity} variant="destructive">
            {loadError}
          </Notice>
        </Stack>
      </PageShell>
    ...
    <PageShell width="full">
      <Stack gap="xl">
        ...
        {summary.state === "partial" ? (
          <Notice title="Trainers home is partially degraded" icon={Activity}>
        ...
        {!summary.bridgeConfigured ? (
          <Notice title="Trainers bridge is not configured" icon={Radar}>
            Live catalog actions may be unavailable until the Trainers bridge configuration is restored.
          </Notice>
        ) : null}
        ...
        {summary.state === "empty" ? (
          <EmptyState
            icon={Rocket}
            title={summary.configured ? "Trainers is ready but quiet" : "Trainers is not configured for this unit yet"}
            description={summary.configured
              ? "There is no active Trainers review, live-catalog, or project-board pressure right now."
              : "The destination exists in the product surface now, but this unit still needs its Trainers workflow configured or seeded."}
            tone="review"
            primaryAction={(
              ...
              <Stack gap="sm">
                ...
                {summary.fetchHealth.sources.filter((source) => source.status !== "ok").map((source) => (
                  <UnifiedCardSection key={source.key} tone="review">
                    ...
                    <BodyText>{source.message || "This source is degraded. Other Trainers actions remain available."}</BodyText>
                  </UnifiedCardSection>
                ))}
              </Stack>
            </UnifiedCardBody>
