...
type ClassScoutListingType = "provider" | "meetupGroup";

export type ClassScoutNormalizedListingInput = {
  title: string;
...
export type ClassScoutDiscoveryArtifact = {
  artifactId: string;
  targetId: string;
  searchQuery: string;
  sourceUrl: string;
...
export type ClassScoutLiveListingSummary = {
  id: string;
  type: ClassScoutListingType;
  title: string;
  borough: string;
  neighborhood: string;
...
export async function scoreClassScoutCandidate(input: {
  ...
  if (!config) {
    return { ok: false, status: 503, error: "ClassScout bridge is not configured" };
  }
...
export async function discoverClassScoutCandidates(input: {
  maxTargets?: number;
...
export async function extractClassScoutCandidate(input: {
  discoveryArtifact: ClassScoutDiscoveryArtifact;
}) {
  const config = getClassScoutBridgeConfig();
  if (!config) {
    return { ok: false, status: 503, error: "ClassScout bridge is not configured" };
...
export async function prepareClassScoutCandidateReview(input: {
  normalizedListing: ClassScoutNormalizedListingInput;
  draftId: string;
...
export async function listClassScoutLiveListings(input: {
  companyId: string;
  ...
  const items: ClassScoutLiveListingSummary[] = [];
...
export async function createClassScoutLiveRevision(input: {
  companyId: string;
  listingId: string;
  listingType: ClassScoutListingType;
}) {
  const config = getClassScoutBridgeConfig();
  if (!config) {
    return { ok: false, status: 503, error: "ClassScout bridge is not configured" };
